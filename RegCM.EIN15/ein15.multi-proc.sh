...
shift $(($OPTIND - 1))
#=================================================== 
Start=${1:-1996};End=${2:-2005} 		# default
...
function Download  					# from ICTP
{
	...
	color 1 7 " nc ..." 
    # :TODO:06/27/2014 03:44:45 PM RET:Tang: connection with parent process
...
Pfifo="/labos/le2p/ctang/tmp/$$.fifo" # make a pipe line  named as PID
mkfifo $Pfifo   
	...
	do
		...
		do
			...
			if [ "$TEST" = "1" ]; then color 7 1 "-t for TEST!";
				...
				( Download $year $param $Time; sleep 3; echo >&1 ) &
			fi
